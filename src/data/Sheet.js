...
export default class Sheet {
    ...
    #createHTML() {
        ...
        const createThead = (empty) => {
            ...
            if (empty) {
                for (let i = 0; i < cols; i++) {
                    headersArray.push('Столбец' + (i + 1));
                }
            }
            else {
                headersArray = this.#data[0];
            }
            ...
            ${headersArray.reduce((ths, element) => ths + createTh(element), createTh(headersArray[0]))}
            </tr>
            </thead>`;
        ...
        const createTbody = (empty) => {
            ...
            const createTr = (vals) => {
                ...
                <tr>
                    <td>${curRow++}</td>
                    ${vals.reduce((tds, element) => tds + createTd(element), createTd(vals[0]))}
                </tr>
                `;
                return str;
            }
            ...
            if (empty) {
                ...
                dataArray = this.#data.slice(1);
            }

            let str = `
            <tbody>
                ${dataArray.reduce((trs, element) => trs + createTr(element), createTr(dataArray[0]))}
            </tbody>
            `;
            return str;
        };
    ...
    show() {
        ...
        // Sheet.#container.querySelector('.data__table_shown')?.classList.replace('data__table_shown', 'data__table');
